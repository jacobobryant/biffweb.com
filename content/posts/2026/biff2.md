---
title: Biff 2.0 sneak peak
description: SQLite, effects, auth, Datastar...
slug: biff2
image: https://biffweb.com/cards/biff2.png
published: 2026-04-20T11:25:00 AM
---
...
and [Biff support for XTDB v2 is in pre-release](https://biffweb.com/p/xtdb2-prerelease/). Now that
...
[Yakread](https://yakread.com) (a 10k-LOC article recommender system) to v2 and did a bunch of
...
as quick as I wanted. For the particular queries Yakread runs&mdash;which are mostly row-oriented&mdash;I've
...
to XTDB&mdash;that SQLite is a mutable database&mdash;is mitigated by Litestream, which streams
changes to object storage and lets you restore from (and even [run ad-hoc
queries](https://fly.io/blog/litestream-vfs/) on) historical snapshots saved with 30-second
granularity.
...
* Scaling beyond one machine is an unsolved problem. Litefs can let you put SQLite nodes in a
  ...
  failover](https://fly.io/docs/litefs/backup/#continuous-backup-via-litestream). So you basically
...
[inside-out database](https://martin.kleppmann.com/2015/03/04/turning-the-database-inside-out.html)
...
web app that uses Reitit&mdash;it even comes with a default sign-in page.
...
Two of these Biff libraries that happen to contain some new stuff&mdash;instead of being a
splitting-out of code that was already in Biff&mdash;are
[biff.graph](https://github.com/jacobobryant/biff.graph), which lets you structure your domain model
as a queryable graph, inspired by Pathom; and [biff.fx](https://github.com/jacobobryant/biff.fx),
...
the same as stuff.user\_id”). I implemented a similar thing where you would submit transactions in a
...
rendering](https://biffweb.com/p/understanding-htmx/) and I think it's a particularly good fit for
Biff's solo developer focus. htmx however has a critical flaw: it's too popular. It has 47k github
stars&mdash;that's _half_ of what Tailwind has.

Datastar fixes this problem by being a much younger project&mdash;a niche of a niche. There is a
...
* It works well for [fancy CQRS
  stuff](https://andersmurphy.com/2025/04/07/clojure-realtime-collaborative-web-apps-without-clojurescript.html)
...
about](https://biffweb.com/p/edn-tests/) for a year and a half but now find
unnecessary&mdash;counterproductive, even&mdash;with coding agents. I had started working on some updates to
...
an app that helps me find good [Star Wars: Unlimited](https://starwarsunlimited.com/) decks to play.
